...
export async function GET() {
  try {
    ...
    try {
      ...
      const batchResult = await client.query(`SELECT COUNT(*) as count FROM current_batch`);
      if (parseInt(batchResult.rows[0].count) === 0) {
        await client.query(`INSERT INTO current_batch (current_batch, sold_out_at) VALUES (1, NULL)`);
      }
      
      // 4. Check batches and insert default data if empty
      const batchesResult = await client.query(`SELECT COUNT(*) as count FROM batches`);
